...
class AdminController extends BaseController {
    ...
    public function owners() {
	 ...
	 public function sms() 
	 {
		$title="SMS";
		 ...
		 $waker = DB::select( DB::raw(" SELECT id, email, first_name FROM walker where is_active='1' and phone!='' ") ); //driver
		 $owner =  DB::select( DB::raw(" SELECT id, email, first_name FROM owner  where phone!='' ") ); //passenger
	...
	function sendsms()
	{
		...
		if($type==1)
		{
			...
			{
				$drivers = $_REQUEST['driver']; //walker
				if(!empty($drivers))
				{
					$people=true;
					...
					{
						sms_notification($dri,'walker', $_REQUEST['sms'] )	;
					}
				}
		   }
		}
		elseif($type==2)
		{
			if(isset($_REQUEST['passenger']))
			{
				$passenger = $_REQUEST['passenger']; //owner
				if(!empty($passenger))
				{
					$people=true;
					...
					{
						sms_notification($ps,'owner', $_REQUEST['sms'] )	;
					}
				}
			}
		}
		...
		if($people==false)
		{
			//select people first then send sms
			
			 $error_messages = "Please select at least one user to send sms.";
			 Session::put('msg', $error_messages);
		}
		else
		{
			//send sms code
			 $error_messages = "Message has been send to all users.";
			 Session::put('msg', $error_messages);
		}
